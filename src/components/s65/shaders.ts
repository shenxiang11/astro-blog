export const dynamicEnvVert = /* glsl */ `
varying vec2 vUv;
void main() {
  vUv = uv;
...
export const dynamicEnvFrag = /* glsl */ `
...
varying vec2 vUv;
void main() {
  vec3 a = texture2D(uEnvmap1, vUv).xyz;
  vec3 b = texture2D(uEnvmap2, vUv).xyz;
  gl_FragColor = vec4(mix(a, b, uWeight) * uIntensity, 1.0);
}
`;

export const speedupVert = /* glsl */ `
varying vec2 vUv;
void main() {
  vUv = uv;
...
vec3 pos2col(vec2 ipos) {
  ipos += vec2(9.0, 0.0);
  return vec3(
    random(ipos + vec2(12.0, 2.0)),
    random(ipos + vec2(7.0, 5.0)),
    random(ipos)
  );
}
...
void main() {
  vec2 uv0 = vUv + vec2(-uTime * 0.5, 0.0);
  float noiseMask = (noise2d(uv0 * vec2(3.0, 100.0)) + 1.0) / 2.0;
  noiseMask = pow(clamp(noiseMask - 0.1, 0.0, 1.0), 11.0);
  noiseMask = smoothstep(0.0, 0.04, noiseMask) * 2.0;
  vec3 colorNoiseMask = colorNoise(uv0 * vec2(10.0, 100.0)) * vec3(1.5, 1.0, 400.0);
  ...
  noiseMask *= smoothstep(1.0, 10.0, uSpeed);
