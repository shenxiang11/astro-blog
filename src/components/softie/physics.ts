...
type Mode = { value: number; velocity: number };
...
const mode = (): Mode => ({ value: 0, velocity: 0 });
...
export class JellyPhysics {
  ...
  setConfig({ stiffness, damping }: { stiffness?: number; damping?: number } = {}) {
    if (typeof stiffness === "number" && Number.isFinite(stiffness)) {
      this.config.stiffness = clamp(stiffness, 0, 1);
    }
    if (typeof damping === "number" && Number.isFinite(damping)) {
  ...
  beginGrab(localPoint: Vec3, worldTarget: Vec3) {
  ...
  _validPoint(point: Partial<Vec3> | null | undefined) {
  ...
  deform(x: number, y: number, z: number, out: Vec3) {
    let px = x;
    let py = y;
    let pz = z;
